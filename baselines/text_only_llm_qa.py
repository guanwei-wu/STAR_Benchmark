                ...
                if option_match:
                    ...
                    if "a" in lower_text and "option a" in lower_text:
                        ...
                        if len(self.option_pattern.findall(generated_text)) == 0:
                            print(
                                f"Warning: Could not parse option from output: {generated_text}"
                            )
                        predicted_options.append("A")
...
def evaluate(model, dataloader):
    ...
    for category in category_total:
        ...
        print(
            f"{category}: {category_accuracies[category]:.4f} ({category_correct[category]}/{category_total[category]})"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate LLM QA model on multiple-choice questions"
    )
    ...
    parser.add_argument(
        "--val_json",
        type=str,
        default="~/STAR/data/STAR_val.json",
        help="Path to validation JSON file",
    )
    parser.add_argument(
        "--prompt_template",
        type=str,
        default="~/STAR/baselines/prompt.txt",
    ...
    val_dataset = LLMQADataset(
        json_file=val_json, prompt_template_file=prompt_template_file
    )
    ...
    accuracy, category_accuracies = evaluate(
        model=llm_qa_model, dataloader=val_dataloader
    )
